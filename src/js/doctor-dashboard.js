...
$(document).ready(function () {
  ...
  const logdin = localStorage.getItem("logdin-user");
  const logdinUser = JSON.parse(sessionStorage.getItem("logdin-user"));

  if (token) {
    const decodedToken = jwt_decode(token);
    const userId = decodedToken.userId;
    console.log("doctor for patients >>>> ", logdinUser);
      ...
      fetchDoctorId(userId).then(function (doctorId) {
        if (doctorId) {
          ...
          $("#appointments-link").click(function () {
            fetchPendingAppointments(doctorId?.id);
          });
        } else {
